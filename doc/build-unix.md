...
If you want to build PushCurrency-Qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a PushCurrency-qt executable will be
built by default.
...
```bash
PushCurrency_ROOT=$(pwd)

# Pick some path to install BDB to, here we create a directory within the PushCurrency directory
BDB_PREFIX="${PushCurrency_ROOT}/db4"
mkdir -p $BDB_PREFIX
...
# Configure PushCurrency Core to use our own-built instance of BDB
cd $PushCurrency_ROOT
./autogen.sh
...
2) The release is built with GCC and then "strip PushCurrencyd" to strip the debug
symbols, which reduces the executable size by about 90%.


To Build PushCurrencyd with compile script
---------------------
...
Security
---------------------
To help make your PushCurrency installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, PushCurrency should be built with a non-executable stack
