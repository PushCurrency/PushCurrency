...
int nAnonymizePushCurrencyAmount = 1000;
...
#else
    const char* pszModule = "PushCurrency";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PushCurrency
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PushCurrency
    // Mac: ~/Library/Application Support/PushCurrency
    // Unix: ~/.PushCurrency
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PushCurrency";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "PushCurrency.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "PushCurrencyd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong PushCurrency will not work properly.");
