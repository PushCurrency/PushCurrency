...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("PushCurrency"))
        return false;
...
{
    // Convert PushCurrency:// to PushCurrency:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("PushCurrency://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "PushCurrency:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "PushCurrency.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for PushCurrency.lnk
...
{
    return GetAutostartDir() / "PushCurrency.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a PushCurrency.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=PushCurrency\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the PushCurrency app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add PushCurrency app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("PushCurrency-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  PushCurrency-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("PushCurrency-Qt"));
