...
void Shutdown()
{
    ...
    RenameThread("PushCurrency-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: PushCurrency.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: PushCurrencyd.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 50003)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 50004)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizePushCurrencyamount=<n> " + _("Keep N PushCurrency anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. PushCurrency is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. PushCurrency is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("PushCurrency version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "PushCurrency server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of PushCurrency") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart PushCurrency to complete") << "\n";
    ...
    nAnonymizePushCurrencyAmount = GetArg("-anonymizePushCurrencyamount", 0);
    if(nAnonymizePushCurrencyAmount > 999999) nAnonymizePushCurrencyAmount = 999999;
    if(nAnonymizePushCurrencyAmount < 2) nAnonymizePushCurrencyAmount = 2;
    ...
    LogPrintf("Anonymize PushCurrency Amount %d\n", nAnonymizePushCurrencyAmount);
